...
const S = {
  ...
  spritesBody:  ['NHLE can show real item icons by downloading them from the internet as you browse. Nothing is stored in the app — skip to use colour tiles only.', 'NHLEは閲覧時にインターネットから本物のアイコンを取得して表示できます。アプリには保存されません。スキップすると色タイルのみになります。', 'NHLE 可在瀏覽時從網路下載真實的物品圖示。不會儲存在程式中。略過則只用顏色方塊。', 'NHLE 可在浏览时从网络下载真实的物品图标。不会保存在程序中。跳过则只用颜色方块。', 'NHLE는 검색 시 인터넷에서 실제 아이콘을 받아 표시할 수 있습니다. 앱에 저장되지 않습니다. 건너뛰면 색상 타일만 사용합니다.', "NHLE peut afficher les vraies icônes en les téléchargeant depuis Internet. Rien n'est stocké dans l'app. Ignorez pour n'utiliser que des tuiles de couleur.", 'NHLE kann echte Symbole aus dem Internet laden. Es wird nichts gespeichert. Überspringen nutzt nur Farbkacheln.', 'NHLE puede mostrar iconos reales descargándolos de internet. No se guarda nada en la app. Omite para usar solo fichas de color.', "NHLE può mostrare icone reali scaricandole da internet. Non viene salvato nulla. Salta per usare solo tessere colorate.", 'NHLE kan echte iconen tonen door ze van internet te laden. Er wordt niets opgeslagen. Sla over voor alleen kleurtegels.', 'NHLE может показывать настоящие иконки, загружая их из интернета. Ничего не сохраняется. Пропустите, чтобы использовать только цветные плитки.'],
  downloadSprites: ['Download sprites', '画像をダウンロード', '下載圖示', '下载图标', '이미지 다운로드', 'Télécharger les sprites', 'Sprites herunterladen', 'Descargar sprites', 'Scarica sprite', 'Sprites downloaden', 'Загрузить спрайты'],
  skipSprites:  ['Skip — colour tiles only', 'スキップ（色タイルのみ）', '略過 — 只用顏色方塊', '跳过 — 只用颜色方块', '건너뛰기 — 색상 타일만', 'Ignorer — tuiles de couleur', 'Überspringen — nur Farbkacheln', 'Omitir — solo fichas de color', 'Salta — solo tessere colorate', 'Overslaan — alleen kleurtegels', 'Пропустить — только плитки'],
